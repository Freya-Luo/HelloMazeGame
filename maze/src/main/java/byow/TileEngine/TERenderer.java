...
public class TERenderer {
    ...
    public void renderVisibleXY(TETile[][] world, Position avatar){
        StdDraw.clear(Color.black);
        int numXTiles = world.length;
        int numYTiles = world[0].length;

        int axisX = avatar.getX();
        int axisY = avatar.getY();
        for(int x= 0; x < numXTiles; x++){
            world[x][axisY].draw(x + xOffset, axisY + yOffset);
        }
        for(int y = 0; y < numYTiles; y++){

            world[axisX][y].draw(axisX + xOffset, y + yOffset);
        }
        StdDraw.show();
    }
    public void renderVisibleSquare(TETile[][] world, Position avatar){
        StdDraw.clear(Color.black);
        int axisX = avatar.getX();
        int axisY = avatar.getY();
        ...
        int halfX;
        int halfy;
        if(axisX == 1 || axisX == world.length-2) halfX = 1;
        else halfX = 2;
        if(axisY == 1 || axisY == world[0].length -2) halfy = 1;
        else halfy =2;

        for (int x = axisX - halfX; x < axisX + halfX + 1 ; x += 1) {
            for (int y = axisY - halfy; y < axisY + halfy + 1; y += 1) {
                int effectiveX = Math.max(x, 0);
                int effectiveY = Math.max(y, 0);
                world[x][y].draw(effectiveX + xOffset, effectiveY + yOffset);
            }
        }
        StdDraw.show();
    }
